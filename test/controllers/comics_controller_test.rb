...
class ComicsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create comic" do
    assert_difference('Comic.count') do
      post comics_url, params: { comic: { abstract: @comic.abstract, status: @comic.status, title: @comic.title, year: @comic.year } }
    end
  ...
  test "should update comic" do
    patch comic_url(@comic), params: { comic: { abstract: @comic.abstract, status: @comic.status, title: @comic.title, year: @comic.year } }
